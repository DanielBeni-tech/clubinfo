    ...
    <div>
      <section className="night-panel">
        <div className="mx-auto max-w-6xl px-4 py-14">
          ...
          <p className="mt-3 max-w-2xl text-lg text-accent">L'assistant intelligent des étudiants de SUP'PTIC</p>
        ...
        <div className="mx-auto grid max-w-6xl gap-10 px-4 lg:grid-cols-3">
          <div className="lg:col-span-2">
            ...
            <p className="mt-4 text-muted-foreground">
              Trouver une information administrative ou pédagogique à SUP'PTIC prend souvent plusieurs jours :
              l'information circule entre affichages, groupes de discussion et documents dispersés. SUP'ONE AI répond
              à ce problème avec un assistant conversationnel qui interroge une base documentaire officielle et
              répond en français, en citant sa source.
            </p>
            <h3 className="mt-8 font-display text-lg">Notre approche</h3>
            <p className="mt-3 text-muted-foreground">
              L'assistant repose sur une architecture de recherche augmentée (RAG) : les documents de l'école sont
              découpés, vectorisés et interrogés à chaque question, avant qu'un modèle de langage ne formule une
              réponse ancrée dans ces extraits. Ce choix limite les réponses inventées et rend chaque réponse
              vérifiable.
            </p>
            <h3 className="mt-8 font-display text-lg">Technologies</h3>
            <ul className="mt-3 flex flex-wrap gap-2">
              {["Python", "FastAPI", "LLM", "RAG", "PostgreSQL / pgvector", "React", "TypeScript"].map((t) => (
                <li key={t} className="rounded-md bg-secondary px-2.5 py-1 font-mono text-xs text-secondary-foreground">
                  {t}
                </li>
              ))}
            </ul>
          ...
          <Card className="h-fit">
            <CardContent className="p-6">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Pôle Innovation & Projets (cadrage et données) et Pôle Développement (backend, interface,
                déploiement), avec le soutien du Bureau Exécutif pour les relations avec l'administration.
              </p>
